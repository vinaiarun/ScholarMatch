    ...
    return {
        createUser: function (options, callback) {
            var newuser = new User({
                ...
                login: options.username.toLowerCase(),
                email: options.username.toLowerCase(),
        ...
        updateStatus: function (userId, status, activeStatus, callback) {
            debugger;
            User.findById(userId, function (err, user) {
                ...
                {
                    if(((status != null && status != -1)|| (activeStatus != null && activeStatus != -1))  && (user != null)) {
                        
                        if((status != null) && (status != -1)) {
                            user.status = status;

                            if(user.status == "Profile Approved") {
                        ...
                        user.save(function (err, result) {
                            if (err) {
                                return callback('user status update failed');
                            }
                            callback(null, result);
                        });
                    }
        ...
        listAllActiveUsers: function (options, callback) {

            var finalQuery = {
                $and: [
                        {
                            status: {$regex: /^((?!Exit Survey Complete).)*$/i,}
                        }, 
                        {
                            status: {$regex: /^((?!Exit Survey Completed).)*$/i,}
                        }, 
                        {active: "Active"},
                        {  
            ...
            User.findByObjQueryAndDisplaySelected(finalQuery, display, function (err, users) {
                if (err) {
                    callback(err);
                } else {
                    // console.dir(users);
